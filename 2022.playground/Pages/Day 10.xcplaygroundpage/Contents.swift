...
func loadInput() -> String {
    let inputPath = Bundle.main.path(forResource: "day10", ofType: nil)!
...
func task1(input: String) -> Int {
    ...
    var instructionQueue: [Instruction] = input
        .components(separatedBy: .newlines)
        .filter { !$0.isEmpty }
        .compactMap { row in
            let components = row.components(separatedBy: .whitespaces)
            let value: Int? = components.count == 2 ? Int(components[1]) : nil
            return Instruction(command: components[0], value: value)
        }
    ...
    while !instructionQueue.isEmpty {
        let currentInstruction = instructionQueue.removeFirst()
        print(("cycle", cycle, "X", X, "command", currentInstruction.command, "value", currentInstruction.value))
        if [20, 60, 100, 140, 180, 220].contains(cycle) {
            result += cycle * X
        }
        // We have to read the state here
        switch currentInstruction.command {
        case "noop": ()
        case "addx":
            instructionQueue = [
                Instruction(command: "addx-actual", value: currentInstruction.value)
            ] + instructionQueue
        case "addx-actual":
            // this is not valid until the next operation
...
func task2(input: String) {
    ...
    var crt = [[String]](repeating: [String](repeating: ".", count: 40), count: 6)
